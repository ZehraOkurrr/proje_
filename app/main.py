...
def read_companies(db: Session = Depends(get_db)):
    ...
    return schemas.ResponseModel(status=True, data=companies)

@app.get("/companies/{company_id}", response_model=schemas.ResponseModel[schemas.Company])
def read_company(company_id: int, db: Session = Depends(get_db)):
    company = crud.get_company(db, company_id)
    if not company:
        return JSONResponse(status_code=404, content={"status": False, "message": "Company not found"})
    return schemas.ResponseModel(status=True, data=company)
...
def update_company(company_id: int, company: schemas.CompanyBase, db: Session = Depends(get_db)):
    ...
    if not updated:
        return JSONResponse(status_code=404, content={"status": False, "message": "Company not found"})
    return schemas.ResponseModel(status=True, data=updated)
...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    return schemas.ResponseModel(status=True, data=created)

@app.get("/users/", response_model=schemas.ResponseModel[list[schemas.User]])
def read_users(db: Session = Depends(get_db)):
    users = crud.get_users(db)
    return schemas.ResponseModel(status=True, data=users)
...
def read_user(user_id: int, db: Session = Depends(get_db)):
    ...
    return schemas.ResponseModel(status=True, data=user)
...
def read_products(db: Session = Depends(get_db)):
    ...
    return schemas.ResponseModel(status=True, data=products)
...
def read_product(product_id: int, db: Session = Depends(get_db)):
    ...
    return schemas.ResponseModel(status=True, data=product)
